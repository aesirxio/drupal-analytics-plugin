...
/**
 * Implements Aesirx Admin Config Form class.
 *
 * @since 1.0.0
 */
class AesirxAnalyticsAdminConfigForm extends ConfigFormBase {
   ...
   *
   * @return string
   *   Returns an string with Form ID value.
   */
      ...
      try {
        ...
        $form['check_cli'] = [
          ...
          '#markup' => '<b class="color-error">' . $this->t(
            "You can\'t use internal server. Error: @message", ['@message' => $e->getMessage()]
          ) . '</b>',
          '#states' => [
            'visible' => [
              ':input[name="1st_party_server"]' => ['value' => AesirxAnalyticsInterface::INTERNAL],
  ...
  /**
   * Implement dovnloadCliButton() function.
   *
   * @param array $form
   *   An associative array containing the structure of the form.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  public function downloadCliButton(array &$form, FormStateInterface $form_state) {
    ...
    try {
      ...
      $this->messenger()->addStatus($this->t('Library successfully downloaded.'));
    }
    catch (Throwable $e) {
      $this->messenger()->addError($this->t('Downloading failed: @message', ['@message' => $e->getMessage()]));
    }
  ...
  /**
   * Implements validateInternalIsRequired() function.
   *
   * @param array $element
   *   Array with elemement value.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The current state of the form.
   *
   * @return void
  ...
  /**
   * Implements validateExternalIsRequired() function.
   *
   * @param array $element
   *   Array with elemement value.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  /**
   * Implements validateDomain() function.
   *
   * @param array $element
   *   Array with elemement value.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
